...
import com.coder.house.ventas.online.ventasonline.exception.NoStockException;
...
public class ComprobanteService {
	...
	public ComprobanteDTO save(Comprobante comprobante) {
		...
		try {
			existeStock(comprobante.getLineas());
...
//		comprobanteAGuardar.setLineas(armarLineas(comprobante.getLineas(), comprobanteAGuardar));
	...
	private BigDecimal calcularTotal(Set<Linea> lineas) {
		...
		for (Linea linea : lineas) {
			// Multiplica la cantidad por el precio para obtener el subtotal de la línea
			BigDecimal subtotal = linea.getPrecio().multiply(new BigDecimal(linea.getCantidad()));
			total = total.add(subtotal); // Suma el subtotal al total general
		}
	...
	private void existeStock(Set<Linea> lineas) {
		for (Linea linea : lineas) {
			...
			if (opProducto.isEmpty()) {
				throw new NoStockException("El producto con ID " + productoid + " no existe.");
			}

			// Verifica si la cantidad vendida es mayor que el stock disponible
			if (linea.getCantidad() > opProducto.get().getCantidad() || opProducto.get().getCantidad() <= 0) {
				throw new NoStockException("No hay suficiente stock para el producto con ID " + productoid);
			}
		}
	}
